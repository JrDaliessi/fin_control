...
Estado da `UX-CHART-003`: Dia 2 da `003A` concluído em `TEST_STRATEGY_READY`. Os contratos da barra imediata, navegação GET, cinco valores existentes, fallback de parâmetro repetido, acessibilidade, alvo de 44 px e rolagem confinada estão em RED controlado com 4 falhas esperadas e 23 testes preservados. A `003B` mantém `3M`/`Ano` com agregação server-side; a `003C` mantém `Tudo`/personalizado e drill-down para buckets trimestrais. Históricos longos nunca enviarão movimentos brutos ao browser.
...
Próximo passo: executar o Dia 3 da `UX-CHART-003A` para implementar somente a barra GET server-rendered e o fallback seguro de parâmetro repetido até satisfazer os testes existentes.
