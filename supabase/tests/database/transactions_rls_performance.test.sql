...
select ok(exists (select 1 from pg_indexes where schemaname = 'public' and tablename = 'transactions' and indexname = 'transactions_user_account_id_idx' and indexdef ~ '\(user_id, account_id\)'), 'account foreign key has a covering index');
select ok(exists (select 1 from pg_indexes where schemaname = 'public' and tablename = 'transactions' and indexname = 'transactions_user_category_type_idx' and indexdef ~ '\(user_id, category_id, type\)'), 'category foreign key has a covering index');
