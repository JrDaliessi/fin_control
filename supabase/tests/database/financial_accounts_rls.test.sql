...
values
  (current_setting('test.user_a_id')::uuid, 'Conta do usuário A', 'checking', 10000),
  (current_setting('test.user_b_id')::uuid, 'Conta do usuário B', 'savings', 20000),
  (current_setting('test.anonymous_user_id')::uuid, 'Conta anônima', 'cash', 30000);
...
select is(
  ...
  'user A identity is active'
);

select results_eq(
  $$select count(*)::bigint from public.financial_accounts$$,
  $$values (1::bigint)$$,
  'user A sees only the owned account'
);
...
select lives_ok(
  $$insert into public.financial_accounts
    (user_id, name, type, initial_balance_in_cents)
    values (current_setting('test.user_a_id')::uuid, 'Nova conta A', 'payment', 0)$$,
  'user A inserts an owned account'
);

select throws_ok(
  $$insert into public.financial_accounts
    (user_id, name, type, initial_balance_in_cents)
    values (current_setting('test.user_b_id')::uuid, 'Conta forjada', 'checking', 0)$$,
...
select is(
  ...
  'user B identity is active'
);

select results_eq(
  $$select count(*)::bigint from public.financial_accounts$$,
  $$values (1::bigint)$$,
  'user B sees only the owned account'
);
...
select throws_ok(
  $$insert into public.financial_accounts
    (user_id, name, type, initial_balance_in_cents)
    values (current_setting('test.user_a_id')::uuid, 'Conta forjada B', 'checking', 0)$$,
...
select throws_ok(
  ...
  '42501', null, 'anon cannot select financial accounts'
);

select throws_ok(
  $$insert into public.financial_accounts
    (user_id, name, type, initial_balance_in_cents)
    values (gen_random_uuid(), 'Conta anon', 'cash', 0)$$,
  '42501', null, 'anon cannot insert financial accounts'
);

reset role;
select set_config(
  'request.jwt.claim.sub',
  current_setting('test.anonymous_user_id'),
  true
);
select set_config(
  'request.jwt.claims',
...
select throws_ok(
  $$insert into public.financial_accounts
    (user_id, name, type, initial_balance_in_cents)
    values (current_setting('test.anonymous_user_id')::uuid, 'Nova conta anônima', 'cash', 0)$$,
  '42501', null, 'anonymous Auth user cannot insert an owned account'
);
