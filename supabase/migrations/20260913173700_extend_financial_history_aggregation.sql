create or replace function public.load_financial_history_start()
returns date
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  return (
    select min(movement.occurred_on)
...
revoke all on function public.load_financial_history_start()
  from PUBLIC, anon, authenticated, service_role;
grant execute on function public.load_financial_history_start()
  to authenticated;
...
begin
  ...
  if (p_start_on + interval '60 years')::date < p_end_on then
    raise exception using
      errcode = '22023',
      message = 'financial evolution bucket interval exceeds 60 years';
  end if;
