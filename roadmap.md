...
Estado da `UX-CHART-003`: Dia 3 da `003A` concluído em `IMPLEMENTATION_IN_PROGRESS` e GREEN. O seletor foi substituído pela barra GET `Semana`, `7D`, `Quinzena`, `15D` e `Mês`, com nomes acessíveis completos, `aria-pressed`, indicação ativa não cromática, alvo de 44 px e rolagem confinada. Parâmetros repetidos agora recaem em `month`. A `003B` mantém `3M`/`Ano` com agregação server-side; a `003C` mantém `Tudo`/personalizado e drill-down para buckets trimestrais.
...
Próximo passo: executar o Dia 4 da `UX-CHART-003A` para expandir controladamente estados, orientação de uso e microinterações sem alterar períodos ou infraestrutura.
